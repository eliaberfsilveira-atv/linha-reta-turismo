  ...
  return (
    ...
    >
      ...
      <a href={WA_URL} target="_blank" rel="noopener noreferrer"
        className="hidden md:inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-display font-bold text-xs uppercase tracking-widest transition-all hover:brightness-105 hover:-translate-y-0.5"
        ...
        >
          <div className="flex flex-col px-6 py-4 gap-4">
            {[['Destinos', '/destinos'], ['Blog', '/blog'], ['Ofertas exclusivas', '/captura'], ['Contato', '#contato']].map(([l, h]) => (
              <Link key={l} href={h} onClick={() => setOpen(false)} className="text-white/80 hover:text-white text-sm font-medium py-1">
                {l}
              </Link>
            ))}
            <a href={WA_URL} target="_blank" rel="noopener noreferrer" className="btn-primary justify-center mt-2">
              Falar no WhatsApp
            </a>
          </div>
        </motion.div>
