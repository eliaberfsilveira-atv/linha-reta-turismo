...
export default function HeroContent() {
  return (
    <div className="relative z-10 flex-1 flex flex-col justify-center px-6 md:px-16 pb-24 pt-8">
      <div className="max-w-3xl">
        ...
        <h1 className="font-display font-extrabold text-5xl md:text-7xl text-white leading-[0.95] tracking-tight mb-6">
          <span className="block overflow-hidden">
            <SplitText text="Do sonho" delay={0.1} by="word" />
          </span>
          <span className="block overflow-hidden" style={{ color: '#FFC247' }}>
            <SplitText text="ao destino," delay={0.25} by="word" />
          </span>
          <span className="block overflow-hidden">
            <SplitText text="sem desvio." delay={0.4} by="word" />
          </span>
        </h1>
        ...
        >
          {STATS.map((stat, i) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: 1.15 + i * 0.1, ease }}
            >
              <div className="font-display font-extrabold text-3xl" style={{ color: '#FFC247' }}>
                <Counter target={stat.target} prefix={stat.prefix} suffix={stat.suffix} duration={1600} />
              </div>
