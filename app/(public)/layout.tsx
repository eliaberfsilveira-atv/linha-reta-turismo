...
export default function PublicLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      {/* NAVBAR */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-16 py-5 backdrop-blur-md border-b border-white/10"
        style={{ background: 'rgba(0,26,41,0.85)' }}>
        ...
        <Link href="/" className="flex items-center gap-3">
          ...
          <div className="leading-none font-display">
            <div className="text-white font-extrabold text-lg tracking-tight">LINHA RETA</div>
            <div className="text-lr-ocean text-[9px] font-semibold tracking-[0.3em]">TURISMO</div>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-white/70">
          ...
          <Link href="/captura"  className="hover:text-white transition-colors">Ofertas exclusivas</Link>
        </div>

        <a href={WA} target="_blank" rel="noopener noreferrer"
          className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest"
